...
def art_get_existing_projects(conn):
	try:
		...
		print(f"art - Error trying to get existing projects: {e}")
...
def art_get_projects_in_string(conn):
	try:
		...
		print(f"art - Error trying to get existing projects (str): {e}")
...
def art_get_selected_metrics_in_string(conn):
	try:
		...
		print(f"art - Error trying to get existing metrics (str): {e}")
...
def art_update_all_projects_list(conn):
	try:
		with conn.cursor() as cursor:
			...
			print("art - Updating artemis project list")
			...
			if response.status_code == 200:
				...
				if new_projects:
					...
					print('art - ', len(new_projects), " new projects added to DB")
					print('art - ', new_projects)
					new_log_entry(conn, ("l", "artemis", f"{len(new_projects)} new projects added"))

				else:
					new_log_entry(conn, ("l", "artemis", f"{len(new_projects)} new projects added"))
					print("art - No new artemis projects since last update")	
			else:
				new_log_entry(conn, ("h", "artemis", f"Status code '{response.status_code} when trying to update project list'"))
				print(f"art - Artemis API response code: {response.status_code}")
	except Error as e:
		print(f"art - Error trying to upodate project list: {e}")
...
def art_get_local_project_metrics(conn, project_name):
	try:
		...
		print(f"art - Error trying to get local project metrics: {e}")
...
def art_get_all_unique_metrics(conn):
	try:
		...
		print(f"art - Error trying to get local metrics: {e}")			
...
def art_get_all_available_metrics(conn):
	try:
		...
		print(f"art - Error trying to get local metrics inc dupes: {e}")
	...
	try:
		with conn.cursor() as cursor:
			...
			if new_unique_metrics:
				...
				print(f"art - {len(new_unique_metrics)} new unique metrics added")
				new_log_entry(conn, ("g", "artemis", f"{len(new_unique_metrics)} new unique metrics added"))
			else:
				print("art - No new unique metrics")
				new_log_entry(conn, ("g", "artemis", "No new unique metrics"))
	except Error as e:
		print(f"art - Error trying to update raw data {e}")
...
def art_get_saved_ecosystem_data(conn):
	try:
		...
		print(f"art - Error trying to get local ecosystem data: {e}")
		...
		with conn.cursor() as cursor:
			...
			if response.status_code == 200:
				...
				if new_ecosystems:
					...
					print(f"art - {len(new_ecosystems)} new ecosystems added")
				
				else:
					print("art - No new ecosystems since last update")

			else:
				print('art - ', response.status_code)
	except Error as e:
		print(f"art - Error trying to get api metrics avaiakble: {e}")
...
def art_get_wanted_metrics(conn):
	try:
		...
		print(f"art - Error trying to get wanted metrics: {e}")
...
def art_add_selected_metrics(conn):
	try:
		...
		print(f"art - Error trying to updated wanted metrics: {e}")
...
def art_get_most_recent_metrics_data_date(conn, single_project_name):
	try:
		...
		print(f"art - Error trying to get most recent data update for {single_project_name} : {e}")
		...
		with conn.cursor() as cursor:
			...
			for single_project_name in project_names:
				...
				if response.status_code == 200:
					...
					print(f"art - Updating data for {single_project_name}")
					...
					print(f"art - UH OH, error code: {response.status_code} for {single_project_name}")
					new_log_entry(conn, ("h", "artemis", f"Status code '{response.status_code} for {single_project_name}'"))
	except Error as e:
		print(f"art - Error trying to updat art raw data: {e}")
