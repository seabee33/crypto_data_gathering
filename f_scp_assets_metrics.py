...
WHERE project_name IN (
    'Acala', 'Aptos', 'Arbitrum', 'Avalanche', 'Base', 'Bitcoin', 'Blast', 'BSC', 
    'Cardano', 'Celo', 'Chiliz', 'Conflux', 'Cosmos', 'Cronos', 'Elrond', 'Ethereum', 
    'Fantom', 'Flow', 'Fuse', 'Gnosis', 'Immutable', 'Injective', 'internetcomputer', 
    'Kusama', 'Linea', 'Manta', 'Mantle', 'Mina', 'Multiversx', 'NEAR', 'Near-protocol', 
    'Optimism', 'Polkadot', 'Polygon', 'Polygon_zk', 'Ronin', 'Scroll', 'Sei', 
    'Sei-network', 'Solana', 'Stacks', 'Starknet', 'Sui', 'Terra', 'The Open Network', 
    'Tron', 'Zksync-era', 'Hyperliquid'
)
...
def calculate_metrics_scp(df):
    ...
    df.drop(columns=[
        ...
        'stablecoin_transfer_volume_mean_90d', 'daa_mean_30d', 'fees_sum_30d', 'tvl_sum_30d',
        'dex_volume_sum_30d', 'stablecoin_transfer_volume_mean_30d','tvl_mean_14d',
...
def add_moving_averages_scp(df):
    ...
    for col in cols_to_process:
        ...
        #     df[f'{col} Perc_Change {period}D'] = grouped[col].transform(lambda x: x.pct_change(periods=period))

    return df



def save_to_fact_table_scp(conn):
    df = fetch_j_raw_scp(conn)  # Fetch the data
    print("Data Fetched")
    df = calculate_metrics_scp(df)  # Calculate metrics
    print("Metrics Added")
    df_final = add_moving_averages_scp(df)  # Add moving averages and percentage changes
    print("MA added")
    # df_final = calculate_momentum_scp(df)
    ...
    try:
        with conn.cursor() as cursor:
            ...
            for i in range(0, len(data), batch_size):
                cursor.executemany(
                    f"""
                    INSERT INTO f_scp_assets_metrics ({cols_str})
                    VALUES ({placeholders}) 
            ...
            conn.commit()
            print("Data successfully inserted into f_scp_assets_metrics")
